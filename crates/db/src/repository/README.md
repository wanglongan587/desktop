...
- Concrete repositories map projects, tasks, sessions, skills, configurable agents, worktrees, task diff comments, and project specification source overrides between SQL rows and domain values.
...
Task deletion cascades through its sessions and owned worktree record. Project deletion cascades through tasks, sessions, worktree records, and specification source overrides. A running descendant returns `ResourceInUse`; no partial cascade is committed.
