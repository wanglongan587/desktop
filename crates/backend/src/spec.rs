...
impl SpecApi {
    ...
    pub(crate) fn resolve_source(
        ...
        if relative_path.is_empty() {
            return Err(invalid_spec_source());
        }
...
/// Maps platform-picker containment failures to the localized public source error.
fn spec_source_error(source: ora_fs::WorkspaceFileSystemError) -> BackendError {
    BackendError::with_source(
        ...
        "specification source is invalid",
        source,
    )
}

/// Builds the localized source error for a structurally invalid picker result.
fn invalid_spec_source() -> BackendError {
    BackendError::new(
        ErrorClassification::InvalidRequest,
        PublicError::SpecSourceInvalid(EmptyErrorParams {}),
        "specification source is invalid",
...
#[cfg(test)]
mod tests {
    use super::{EffectiveSource, infer_sources, path_is_within, select_source};
    use ora_contracts::{
        SpecSource, SpecSourceAvailability, SpecSourceOrigin, SpecSourceVisibility, SpecWorkflow,
    };
