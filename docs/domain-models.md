...
Identifiers are newtypes generated by one macro, so a `TaskId` cannot be passed where a `ProjectId` is expected: `ProjectId`, `TaskId`, `WorktreeId`, `SessionId`, `SkillId`, `AgentDefinitionId`, `VirtualFolderId`, `VirtualEntryId`, `ArtifactId`, `WorkflowId`, `WorkflowSnapshotId`, `WorkflowRunId`, `WorkflowNodeRunId`. Each wraps a `String` and serializes as that bare string, so existing JSON and SQLite text boundaries are unchanged.
...
`Session.title` is the durable display name. It is populated only during the bounded first-title acquisition window owned by a newly attached actor; restoring an existing session does not reopen that window. The provider session id and the acquisition/lock state remain backend-only concerns.
