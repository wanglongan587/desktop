...
Identifiers are newtypes generated by one macro, so a `TaskId` cannot be passed where a `ProjectId` is expected: `ProjectId`, `TaskId`, `WorkspaceId`, `GitCleanupJobId`, `WorktreeProvisioningLeaseId`, `SessionId`, `SkillId`, `AgentDefinitionId`, `WorkflowId`, `WorkflowSnapshotId`, `WorkflowRunId`, and `WorkflowNodeRunId`. Each wraps a `String` and serializes as that bare string, so existing JSON and SQLite text boundaries are unchanged. A `Worktree` uses its owning `WorkspaceId` as its identity.
...
Optional fields exist only where the schema column is nullable — for example `Worktree.branch_name`. Non-nullable columns stay required in both struct fields and constructors.
