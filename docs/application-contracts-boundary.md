...
- Workspace file paths are validated as relative paths, canonicalized before containment checks, and returned with slash separators. The filesystem crate bounds file reads and search output, creates only missing empty files or directories, copies/moves/deletes without following symbolic links, and reports native watcher changes as cache-invalidating batches. See [Task Workspace Files](task-workspace-files.md).
