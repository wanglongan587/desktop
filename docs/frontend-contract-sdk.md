...
Namespaces are `project`, `projectWorkContext`, `task`, `session`, `agentRuntime`, `skill`, `agent`, `fileSystem`, `spec`, and `gitIdentity`. The `spec` namespace keeps unary catalog/read/configuration calls and its workspace-event stream behind one target-tagged API.
