...
function ImageBlock({
  ...
  if (
    src === null ||
    !PREVIEWABLE_IMAGE_TYPES.has(content.mimeType.toLocaleLowerCase())
  ) {
    return (
      <BinaryDownload
  ...
  const label = content.uri
    ? resourceName(content.uri)
...
function AudioBlock({ content }: { content: acp.AudioContent }) {
  ...
  return (
    <section
      className="max-w-xl rounded-md border border-border bg-muted/20 p-3"
      aria-label={t("chat.content.audio")}
    >
...
function EmbeddedResourceBlock({ content }: { content: acp.EmbeddedResource }) {
  ...
  if ("text" in resource) {
    ...
    return (
      <details
        className="group max-w-2xl overflow-hidden rounded-md border border-border bg-muted/20"
        open
      >
        ...
        <pre
          data-selectable
          className="max-h-80 overflow-auto border-t border-border bg-[var(--code-background)] px-3 py-2.5 text-[11px] leading-5 whitespace-pre-wrap"
        >
          {resource.text}
        </pre>
      </details>
  ...
  if (
    resource.mimeType &&
    PREVIEWABLE_IMAGE_TYPES.has(resource.mimeType.toLocaleLowerCase())
  ) {
    return (
      <ImageBlock
        content={{
  ...
  if (resource.mimeType?.startsWith("audio/")) {
    return (
      <AudioBlock
        content={{ data: resource.blob, mimeType: resource.mimeType }}
      />
    );
  }
  return (
