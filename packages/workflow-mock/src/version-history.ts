...
export function createMockWorkflowVersions(workflows: DemoWorkflow[]): MockWorkflowVersions {
...
): ReactFlowJsonObject<Node<WorkflowNodeData, "workflow">, Edge> {
...
function previousGraphSnapshot(
  ...
  const removableNode = [...graph.nodes].reverse().find((node) => node.data.kind !== "start");
  ...
  return {
    ...
    edges: graph.edges.filter(
      (edge) => edge.source !== removableNode.id && edge.target !== removableNode.id,
    ),
  };
}
