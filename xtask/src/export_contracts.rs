...
pub fn check_exported_contracts(workspace_root: &Path) -> Result<(), Box<dyn std::error::Error>> {
    check_generated_paths(
        ...
        |isolated_workspace| {
            copy_contract_source_tree(workspace_root, isolated_workspace)?;
            run_export_contracts(isolated_workspace)
        },
    )
}

/// Seeds hand-written contract runtime modules before isolated generation replaces owned files.
fn copy_contract_source_tree(
    workspace_root: &Path,
    isolated_workspace: &Path,
) -> Result<(), Box<dyn std::error::Error>> {
    let relative_source = Path::new("packages/contracts/src");
    copy_directory_tree(
        &workspace_root.join(relative_source),
        &isolated_workspace.join(relative_source),
    )
}

/// Recursively copies a directory without relying on platform-specific shell commands.
